"""Default message templates and the AI system prompt.

Kept in their own module so both service.py and settings.py can use them as
defaults without an import cycle. These are the *defaults*; the admin page can
override them at runtime (the overrides live in the database).
"""
...
# Placeholders available to the editable mini-report template, shown in the
# admin UI so users know what they can use.
MINI_TEMPLATE_FIELDS = [
    "timestamp", "client", "server", "device_count",
    "download", "upload", "ping", "download_mb", "upload_mb", "status_text",
]

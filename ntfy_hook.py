...
class Bot:
    ...
    `token` is an optional bearer access token for protected topics."""
    ...
    @classmethod
    def init(cls, url: str, token: str, timeout: int) -> "Bot":
        if url.strip() == "":
            raise ValueError("ntfy URL cannot be empty")
        return cls(url.strip(), token.strip(), timeout)
    ...
    def send_message(self, message: str, parse_mode: str = "HTML") -> str:
        ...
        response = requests.post(self.url, data=body, headers=self._headers(), timeout=self.timeout)
